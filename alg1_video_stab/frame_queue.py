...
class FrameQueue:
    ...
    def set_frame_source(self, source, oursourceFrames = None):
        if oursourceFrames!= None:
            self.given_frames = oursourceFrames
    ...
    def read_frame(self, pop_ind=True, array=None):
        if self.given_frames!= None:
            if (len(self.given_frames) > 0):
                frame = self.given_frames[0]
                self.given_frames.pop(0)
    ...
    def _append_frame(self, frame, pop_ind=True, existing = True):
        popped_frame = None
